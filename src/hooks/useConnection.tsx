...
type TokenGeneratorData = {
  ...
  connect: (mode: ConnectionMode, metadata?: string) => Promise<void>;
};
...
export const ConnectionProvider = ({
  ...
  const connect = useCallback(
    async (mode: ConnectionMode, metadata?: string) => {
      let token = "";
      let url = "";
      if (mode === "cloud") {
        try {
        ...
        const { accessToken } = await fetch("/api/token", {
          method: "POST",
          body: JSON.stringify({ metadata: metadata || "default_metadata" }),
        }).then((res) => res.json());
        token = accessToken;
      } else {
